...
渲染层可以通过 [WXML](/模块/视图层/WXML/README.md) 对数据进行绑定。
...
除了初始化数据和生命周期函数，Page 中还可以定义一些特殊的函数：事件处理函数。在渲染层可以在组件中加入[事件绑定](/模块/视图层/WXML/事件.md)，当达到触发事件时，就会执行 Page 中定义的事件处理函数。
...
| 页面返回 | 调用 API `wx.navigateBack`或用户按左上角返回按钮 | onShow | onUnload |
